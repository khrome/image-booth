...
class FloodFill extends _tool.Tool {
  ...
  paint(pixels, x, y, brush, controls) {
    ...
    while (stack.length > 0) {
      ...
      x = item[0];
      y = item[1];
      if (!hits[pos] && colorDiff(sourceColor[0], sourceColor[1], sourceColor[2], pixels.data[pos], pixels.data[pos + 1], pixels.data[pos + 2]) < options.amount / 100) {
        pp++;
